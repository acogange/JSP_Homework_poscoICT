...
public class BbsDao {
	...
	public boolean writeBbs(BbsDto dto) {
		...
		return count>0?true:false; //데이터베이스 오류
	}
	...
	public boolean answer(int seq, BbsDto bbs) {//부모의 번호, 새로운 댓글
		//UPDATE 후 INSERT
		String sql1=" UPDATE BBS SET STEP=STEP+1 WHERE REF=( SELECT REF FROM (SELECT REF FROM BBS a WHERE SEQ=?) A) "	//해당 글과 그룹 번호가 같고
